...
Ren'Py поддерживает скины для лаунчера, то есть изменение его
внешнего вида. Для этого выполните следующие шаги:

Скины специфичны для используемой версии Ren'Py и не могут
гарантировать прямую или обратную совместимость.

1. Откройте лаунчер в лаунчере. Это можно сделать на экране
   настроек (preferences).

2. Создайте файл скрипта :file:`skin.rpy`.

3. Скопируйте следующее в :file:`skin.rpy`::
...
4. Измените :file:`skin.rpy` для оформления лаунчера. Поместите используемые
   файлы изображений в папку game лаунчера. Рекомендуемый размер
   фона — 800x600 пикселей.

5. Выберите тему Custom в настройках.

Неправильный файл :file:`skin.rpy` может помешать запуску лаунчера.
Чтобы исправить это, вам нужно будет удалить :file:`skin.rpy` и
:file:`skin.rpyc` из папки game лаунчера, запустить лаунчер, а
затем вернуть их на место.